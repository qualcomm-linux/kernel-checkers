...
exit_status=0
log_summary=()
dt_dir="arch/arm64/boot/dts/qcom"
...
# by the head_sha
git checkout $base_sha > /dev/null 2>&1
dtb_files=$(kmake_image_make -j$(nproc) O=temp-out dtbs grep -oP 'arch/arm64/boot/dts/qcom/.*?\.dtb')
...
for devicetree in $dtb_files; do
    echo "Checking $devicetree"
    if kmake_image_make -j"$(nproc)" O="$temp_out" CHECK_DTBS=y "$devicetree" |& tee "$log_file" | grep -q "$devicetree"; then
